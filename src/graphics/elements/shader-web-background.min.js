...
(() => {
	...
	function B(a, b, c) {
		y(
			...
			'Shader source element of id "' + c + '" should be of type: <script type="' + (b + '" id="' + c + '">'),
		);
	}
	function D(a) {
		const b = document.getElementById(a);
		y(b, 'Missing shader source: <script type="frag" id="' + (a + '">'));
		B(b, "frag", a);
		return b.text;
	}
	function E(a) {
		a += "Vertex";
		const b = document.getElementById(a);
		return b ? (B(b, "vert", a), b.text) : "attribute vec2 V;void main(){gl_Position=vec4(V,0,1);}";
	}
	function F(a, b) {
	...
	class G {
		constructor(a, b, c, d) {
			...
			this.i = () => {
				...
				f = a.gl;
				for (h = 0; h < a.g; h++) f.activeTexture(f.TEXTURE0 + h), f.bindTexture(f.TEXTURE_2D, null);
				a.g = 0;
			};
		}
	}
	...
	function J(a, b, c, d, l, f) {
		...
		const p = H(a),
			...
			g = {
				...
				toShaderX: (e) => (e - a.getBoundingClientRect().left) * g.cssPixelRatio + 0.5,
				toShaderY: (e) => a.height - (e - a.getBoundingClientRect().top) * g.cssPixelRatio - 0.5,
		...
		for (const e in b) {
			...
			var u = Object.keys(n);
			for (const k of m.m)
				y(n[k.name], 'No configuration for uniform "' + k.name + '" defined in shader "' + e + '"'),
