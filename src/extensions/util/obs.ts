...
// 		try {
// 			const sceneList = await this.call('GetSceneList');
// 			const scene = sceneList.scenes.find((s) => (s?.name?.toString()?.startsWith(name)));
// 			if (scene) {
// 				await this.call('SetCurrentProgramScene', { sceneName: scene.name?.toString()! });
// 			} else {
...
// 		try {
// 			if (useDecibel) {
// 				await this.call('SetInputVolume', { inputName: source, inputVolumeDb: volume });
// 			} else {
...
// 	 */
// 	async setSceneItemProperties(scene: string, source: number, itemProperties: any): Promise<void> {
// 		if (!ncgOBSConfig.enabled) {
// 			// OBS not enabled, don't even try to set.
// 			// throw new Error('No OBS connection available');
// 			nodecg.log.warn(`[OBS] No OBS connection`);
...
// 			// await this.call('SetSceneItemTransform', { 'scene-name': scene, item: { name: source }, ...itemProperties });
// 			await this.call('SetSceneItemTransform', { sceneName: scene, sceneItemId: source, sceneItemTransform: itemProperties });
...
async function connect(): Promise<void> {
	try {
		const { obsWebSocketVersion, negotiatedRpcVersion } = await obs.connect(`ws://${ncgOBSConfig.ip}:${ncgOBSConfig.port}`, ncgOBSConfig.password, {
			eventSubscriptions: EventSubscription.All | EventSubscription.InputVolumeMeters,
			rpcVersion: 1
		});

		nodecg.log.info(`[OBS] Connection successful | Version ${obsWebSocketVersion} (using RPC ${negotiatedRpcVersion})`);
